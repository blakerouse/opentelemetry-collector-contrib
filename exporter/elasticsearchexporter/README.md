...
- `sending_queue`: Configures the queueing and batching behaviour. Below are the defaults (which may vary from standard defaults), for full configuration check the [`exporterhelper` docs][exporterhelper].
  ...
  - `batch`:
    ...
    - `sizer` (default=bytes): Size batches by bytes. Bytes measure the serialized JSON document bodies, excluding the small per-document bulk action metadata lines, so the actual NDJSON bulk request is slightly larger than the configured sizes. To address this discrepancy, the bulk indexers also flush whenever their exact NDJSON size (including action metadata) exceeds the configured max_size.
